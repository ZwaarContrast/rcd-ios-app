...
//    NSUUID *id = [NSUUID alloc] ;
//    [id initWithUUIDString:@"F7826DA6-4FA2-4E98-8024-BC5B71E0893E"];
//    
//    [[NotificarePushLib shared] startMonitoringBeaconRegion:id];
//    
...
- (void)application:(UIApplication *)application didRegisterForRemoteNotificationsWithDeviceToken:(NSData *)deviceToken {
    ...
    [[NotificarePushLib shared] registerDevice:deviceToken withUserID:deviceId withUsername:deviceName completionHandler:^(NSDictionary *info) {
        
        [[NotificarePushLib shared] startLocationUpdates];
        [self addTags];
        
        NSLog([NSString stringWithFormat:@"%@/%@/%@", @"Die shit is geregistreerd ", deviceName, deviceId]);
...
- (void)notificarePushLib:(NotificarePushLib *)library monitoringDidFailForRegion:(CLRegion *)region withError:(NSError *)error{
    ...
    [[[NotificarePushLib shared] locationManager] startMonitoringForRegion:region];
}
...
- (void)notificarePushLib:(NotificarePushLib *)library didRangeBeacons:(NSArray *)beacons inRegion:(CLBeaconRegion *)region{
    NSLog(@"Beacons: %lu", [beacons count]);
    for(NSDictionary * item in beacons) {
       [[NotificarePushLib shared ] openBeacon: item];
        NSLog(@"Beacon: %@", item);
    }
